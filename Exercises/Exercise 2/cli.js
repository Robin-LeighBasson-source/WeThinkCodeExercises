...
program
  ...
  .option('--sort <strategy>', 'Sort strategy: importance, priority, deadline, newest, oldest', 'importance')
  .action((options) => {
    const tasks = taskManager.listTasks(options.status, options.priority, options.overdue, options.sort);
    if (tasks.length > 0) {
      tasks.forEach(task => {
