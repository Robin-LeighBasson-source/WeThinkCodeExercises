...
const { applySortStrategy } = require('./utils/sorting');
...
class TaskManager {
  ...
  listTasks(statusFilter = null, priorityFilter = null, showOverdue = false, sort = 'importance') {
    // Support both old and new calling conventions
    // Old: listTasks(statusFilter, priorityFilter, showOverdue)
    // New: listTasks({ status: 'todo', priority: 2, overdue: false, sort: 'importance' })
    ...
    let tasks;
    if (options.overdue) {
      tasks = this.storage.getOverdueTasks();
    } else if (options.status) {
      tasks = this.storage.getTasksByStatus(options.status);
    } else if (options.priority) {
      tasks = this.storage.getTasksByPriority(parseInt(options.priority));
    } else {
      tasks = this.storage.getAllTasks();
    }

    // Apply sorting strategy
    const sortStrategy = options.sort || 'importance';
    try {
      tasks = applySortStrategy(tasks, sortStrategy);
    } catch (error) {
      console.error(`Warning: ${error.message}. Using default importance sort.`);
      tasks = applySortStrategy(tasks, 'importance');
    }
